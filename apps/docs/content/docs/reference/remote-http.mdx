...
# @converge/remote-http

HTTP transport implementation of the `Remote` interface. Connects clients to Converge servers over HTTP.

## Installation

```bash
pnpm add @converge/remote-http
```
...
```ts
function createHttpRemote<S extends ConvergeSchema>(
