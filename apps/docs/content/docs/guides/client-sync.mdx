...
This guide shows how to set up client-side sync with a Converge server.
...
import type { Store, DbEvent } from '@converge/client';
import type { Change, EntityName } from '@converge/core';
import type { AppSchema } from './schema';
...
import { createReplicator, InMemoryOutbox } from '@converge/client';
import { createHlcState, tickHlc, makeUpsert, makeDelete } from '@converge/core';
...
<Callout type="info">
No manual conflict resolution needed. Converge's field-level LWW ensures consistent, deterministic merges across all clients.
</Callout>
