...
<Diagram
  ...
  nodes={[
    ...
    { id: 'tags-read', relativeTo: 'load', anchor: 'right', offset: { x: 40 }, variant: 'label', label: 'converge_tags' },
    { id: 'todos', relativeTo: 'domain', anchor: 'right', offset: { x: 40 }, variant: 'label', label: 'todos' },
    { id: 'tags-write', relativeTo: 'tags', anchor: 'right', offset: { x: 40 }, variant: 'label', label: 'converge_tags' },
  ]}
  edges={[
    { from: 'append', to: 'load' },
    { from: 'load', to: 'lww' },
    { from: 'lww', to: 'domain' },
    { from: 'domain', to: 'tags' },
    { from: 'tags-read', to: 'load', fromSide: 'left', toSide: 'right', dashed: true, animated: true },
    { from: 'domain', to: 'todos', fromSide: 'right', toSide: 'left', dashed: true, animated: true },
    { from: 'tags', to: 'tags-write', fromSide: 'right', toSide: 'left', dashed: true, animated: true },
  ]}
/>
...
<Callout type="info">
**Without fieldMap**, only the `converge_tags` table is updated. The domain table is unchanged.

**With fieldMap**, both `converge_tags` and the domain table are updated.
</Callout>
...
// Existing state (from converge_tags)
{
...
The materializer maintains a `converge_tags` table:
