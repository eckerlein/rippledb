...
Let's build a simple todo backend with Converge. By the end, you'll have a server that:
...
npm init -y
npm install @converge/core @converge/db-sqlite @converge/materialize-db better-sqlite3
...
Create `src/db.ts` to configure your Converge database:
