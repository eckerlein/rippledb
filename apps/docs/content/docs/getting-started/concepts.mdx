...
<Callout type="info">
Server-side materialization is optional. It's useful if your server also serves 
direct queries (REST APIs, admin dashboards) alongside the sync endpoints.
On the client, the `Store` handles applying changes to local state.
</Callout>
...
<Diagram
  ...
  nodes={[
    ...
    { id: 'server', relativeTo: 'core', anchor: 'right', offset: { x: 40, y: -180 }, label: ['server', '(Db interface)'] },
    { id: 'client', relativeTo: 'core', anchor: 'right', offset: { x: 40, y: 0 }, label: ['client', '(Store/Sync)'] },
    { id: 'materializeCore', relativeTo: 'core', anchor: 'right', offset: { x: 40, y: 180 }, label: ['materialize-core', '(projectors)'] },
    { id: 'db', relativeTo: 'server', anchor: 'right', offset: { x: 40 }, label: ['db-*', '(adapters)'] },
    { id: 'store', relativeTo: 'client', anchor: 'right', offset: { x: 40, y: -80 }, label: ['store-*', '(adapters)'] },
    { id: 'controllers', relativeTo: 'client', anchor: 'right', offset: { x: 40 }, label: ['client-controllers', '(CRUD)'] },
    { id: 'bindTanstack', relativeTo: 'client', anchor: 'right', offset: { x: 40, y: 80}, label: ['bind-tanstack-query', '(UI Cache)'] },
    { id: 'materialize', relativeTo: 'materializeCore', anchor: 'right', offset: { x: 40 }, label: ['materialize-*', '(adapters)'] },
    { id: 'clientQuery', relativeTo: 'controllers', anchor: 'right', offset: { x: 40, y: 40 }, label: ['client-query', '(Final DX)'] },
  ]}
  edges={[
    { from: 'core', to: 'server', orthogonal: true, fromSide: 'bottom', toSide: 'left' },
    { from: 'core', to: 'client', orthogonal: true, fromSide: 'right', toSide: 'left' },
    { from: 'core', to: 'materializeCore', orthogonal: true, fromSide: 'bottom', toSide: 'left' },
    { from: 'server', to: 'db', orthogonal: true, fromSide: 'right', toSide: 'left' },
    { from: 'client', to: 'store', orthogonal: true, fromSide: 'right', toSide: 'left' },
    { from: 'client', to: 'controllers', orthogonal: true, fromSide: 'right', toSide: 'left' },
    { from: 'client', to: 'bindTanstack', orthogonal: true, fromSide: 'right', toSide: 'left' },
    { from: 'materializeCore', to: 'materialize', orthogonal: true, fromSide: 'right', toSide: 'left' },
    { from: 'controllers', to: 'clientQuery', orthogonal: true, fromSide: 'right', toSide: 'left' },
    { from: 'bindTanstack', to: 'clientQuery', orthogonal: true, fromSide: 'right', toSide: 'left' },
  ]}
/>
