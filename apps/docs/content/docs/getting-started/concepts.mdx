---
title: Core Concepts
description: Understand the mental model behind Converge — HLCs, Changes, Streams, and Materialization
---
...
Converge is built on a few key ideas. Understanding them will help you use the library effectively.
...
```ts
import { createHlcState, tickHlc, compareHlc } from '@converge/core';
...
```ts
import { makeUpsert, makeDelete } from '@converge/core';
...
Converge uses **Last-Writer-Wins (LWW)** at the field level, not the row level.
...
2. Materializer reads existing tags from `converge_tags`
3. For each field, compare tags and apply LWW
4. Update the `todos` table with winning values
5. Update `converge_tags` with new timestamps
...
- [API Reference](/docs/reference/core) — Full `@converge/core` API
