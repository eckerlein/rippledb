...
<Callout type="info">
  For a higher-level API that combines these controllers with automatic cache
  invalidation for TanStack Query, see the
  [client-query](/docs/reference/client-query) package.
</Callout>
