...
A production-ready SQLite adapter for Converge using [better-sqlite3](https://github.com/WiseLibs/better-sqlite3).
...
<Tabs items={['pnpm', 'npm', 'yarn']}>
  <Tab value="pnpm">
```bash
pnpm add @converge/db-sqlite better-sqlite3
```
  </Tab>
  <Tab value="npm">
```bash
npm install @converge/db-sqlite better-sqlite3
```
  </Tab>
  <Tab value="yarn">
```bash
yarn add @converge/db-sqlite better-sqlite3
```
  </Tab>
</Tabs>
...
<Callout type="info">
When using `db`, Converge will **not** close the connection when you call `db.close()`. You're responsible for managing the connection lifecycle.
</Callout>
...
You can use `db-sqlite` with Drizzle for your domain queries while Converge manages the sync:
...
// Use SqliteDb for Converge sync
const convergeDb = new SqliteDb<MySchema>({
...
// Now you can use drizzleDb for queries and convergeDb for sync
