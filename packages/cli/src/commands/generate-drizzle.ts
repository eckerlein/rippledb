...
export async function generateFromDrizzle(
  ...
  try {
    ...
    logger.error?.(
      `Install it with: ${getInstallCommand("drizzle-orm", {
        dev: true,
        cwd,
      })}`,
    );
  ...
  logger.log?.(
    `Found ${tables.length} table${tables.length === 1 ? "" : "s"}: ${tables
      .map((t) => t.name)
      .join(", ")}`,
  );
  ...
  for (const { name, table } of tables) {
    ...
    for (const [columnName, column] of Object.entries(columns)) {
      const fieldType = drizzleTypeToRipple(
        column.dataType,
        column.notNull,
        logger.warn,
      );
      fieldDefinitions.push(
        `    ${formatPropertyName(columnName)}: ${fieldType},`,
      );
    }
