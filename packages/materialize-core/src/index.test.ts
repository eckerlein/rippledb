import { type ChangeTags, makeDelete, makeUpsert } from "@rippledb/core";
...
describe("applyChangeToState", () => {
  it("keeps newer field values when older tags arrive", () => {
    const first = applyChangeToState(
      null,
      upsert("2:0:a", { title: "new", done: false }),
    );
  ...
  it("applies deletes when the delete tag is newer", () => {
    const first = applyChangeToState(
      null,
      upsert("1:0:a", { title: "keep", done: false }),
    );
